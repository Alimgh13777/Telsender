...
class TelegramSenderApp:
    ...
    async def main_async(self):
        try:
            ...
            if not any(acc["authorized"] for acc in self.accounts):
            ...
            while self.targets and not self.stop_flag.is_set():
                ...
                for _ in range(batch_sz):
                    if not self.targets:
                        break
                    batch.append(self.targets.popleft())
                if not batch:
                    break
                self.log_ui(f"--- Batch {len(batch)} messages ---", "info")

                tasks = []
                account_index = 0
                for target in batch:
                    acc = active[account_index % len(active)]
                    account_index += 1
                    delay = random.uniform(min_d, max_d)
                    tasks.append(self._send_with_account(acc, target, delay, sent_total + len(tasks), retries))

                results = await asyncio.gather(*tasks, return_exceptions=True)
                success_count = sum(1 for r in results if r is True)
                sent_total += success_count
                self.root.after(0, lambda v=sent_total: self.progress.configure(value=v))

                await self._sleep_or_abort(batch_int)

            self.log_ui(f"🏁 Done. Total successfully sent: {sent_total}")
        ...
        for acc in self.accounts:
            ...
            try:
                ...
                if not await client.is_user_authorized():
                    ...
                    if not code: raise Exception("Login cancelled")
                    try:
                        await client.sign_in(phone, code)
                    except SessionPasswordNeededError:
                        pwd = await self.login_helper.request_password(f"2FA for {masked}:")
                        if not pwd: raise Exception("Login cancelled")
    ...
    async def _send_with_account(self, acc, target, delay, msg_counter, retries):
        ...
        for attempt in range(retries):
            ...
            try:
                ...
                self.log_ui(f"⚠ Network error {masked}: {e}", "warn")
                try:
                    await client.connect()
